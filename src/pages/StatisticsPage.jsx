...
function StatisticsPage() {
    const categoryMap = Object.fromEntries(categories.map(cat => [cat.value, {label: cat.label, color: cat.color}]));
    ...
    const categoryData = subscriptionsStore.data.reduce((acc, item) => {
        ...
        if (!acc[categoryInfo.label]) {
            acc[categoryInfo.label] = { category: categoryInfo.label, items: [], count: 0, color: categoryInfo.color};
        }
    ...
    useEffect(() => {
        ...
        console.log(subscriptionsStore.data)
